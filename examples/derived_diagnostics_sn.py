...
# -*- coding: utf-8 -*-
"""
Post-process SN-only (Pantheon+SH0ES, z>0.023) MCMC to compute standard diagnostics:
q(z), w_eff(z), statefinders {r,s}, and Om(z).

- Uses outputs/sn_chain_mcmc_pantheon.npy (walkers x steps x 6) with params [Ωm, ε, ω, ϕ, γ, H0]
- No re-fitting. Pure math on existing samples.
- ΛCDM comparator (H0, Ωm) is taken from:
    1) outputs/sn_lcdm_fit_summary.json → ["ΛCDM"]["parameters"] (if you have one)
    2) outputs/sn_mcmc_pantheon_summary.json → ["best_fit"] (fallback if it contains Ωm,H0)
    3) chain medians (last resort)
- Saves → outputs/sn_diagnostics.json
"""
...
CHAIN_FILE = os.path.join(OUT_DIR, "sn_chain_mcmc_pantheon.npy")
SN_SUMM    = os.path.join(OUT_DIR, "sn_mcmc_pantheon_summary.json")          # optional
SN_LCDM    = os.path.join(OUT_DIR, "sn_lcdm_fit_summary.json")               # optional
OUT_JSON   = os.path.join(OUT_DIR, "sn_diagnostics.json")
...
def diagnostics_from_H(H, z, H0):
    E   = H / H0
    dE  = np.gradient(E,  z, edge_order=2)  # 2nd-order edges → accurate r0,s0
    ...
    # Om(z): undefined at z=0 → safe compute and leave NaN at 0
    den = (1.0 + z)**3 - 1.0
...
# -----------------------
# ΛCDM diagnostics (use SN summaries if present; else medians)
# -----------------------
H0_LCDM = None
Om_LCDM = None

# 1) Preferred: sn_lcdm_fit_summary.json → ["ΛCDM"]["parameters"]
if os.path.exists(SN_LCDM):
    try:
        with open(SN_LCDM, "r", encoding="utf-8") as f:
            Jlcdm = json.load(f)
        H0_LCDM = (try_get(Jlcdm, "ΛCDM", "parameters", "H₀") or try_get(Jlcdm, "ΛCDM", "parameters", "H0"))
        Om_LCDM = (try_get(Jlcdm, "ΛCDM", "parameters", "Ωₘ") or try_get(Jlcdm, "ΛCDM", "parameters", "Om"))
    except Exception:
        H0_LCDM, Om_LCDM = None, None

# 2) Fallback: sn_mcmc_pantheon_summary.json → ["best_fit"] (if contains Ωm,H0)
if (H0_LCDM is None or Om_LCDM is None) and os.path.exists(SN_SUMM):
    try:
        with open(SN_SUMM, "r", encoding="utf-8") as f:
            Jsn = json.load(f)
        H0_LCDM = H0_LCDM or try_get(Jsn, "best_fit", "H₀") or try_get(Jsn, "best_fit", "H0")
        Om_LCDM = Om_LCDM or try_get(Jsn, "best_fit", "Ωₘ") or try_get(Jsn, "best_fit", "Om")
    except Exception:
        pass

# 3) Last resort: medians from the SN chain
if H0_LCDM is None: H0_LCDM = float(np.nanmedian(samples[:,5]))
if Om_LCDM is None: Om_LCDM = float(np.nanmedian(samples[:,0]))

H0_LCDM = float(H0_LCDM); Om_LCDM = float(Om_LCDM)
...
print("\nSN-only diagnostics (Genesis, Ωm free):")
...
print("\nΛCDM diagnostics (H0, Ωm from SN summary or chain medians):")
